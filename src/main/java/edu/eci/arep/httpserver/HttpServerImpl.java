...
public class HttpServerImpl implements HttpServer {
    ...
    public void startServer() throws IOException {
        ...
        try {
            ...
            nanoSpark.get("/cos", this::getCosHandler);
            nanoSpark.get("/sin", this::getSinHandler);
            nanoSpark.get("/tan", this::getTanHandler);
    ...
    private boolean isSparkEndpoint(String endpoint, NanoSpark nanoSpark) {
        ...
        if (endpoint.contains("/cos") | endpoint.contains("/sin") | endpoint.contains("/tan")) {
            nanoSpark.check(endpoint);
            isSparkEndpoint = true;
        }
        if (endpoint.contains("/calculadora")) {
            endpoint = endpoint.replace("/calculadora", "/");
            nanoSpark.check(endpoint);
            isSparkEndpoint = true;
        }
    ...
    private String getCosHandler(Request req, Response res) {
        String result = null;
        try {
            result = persistenceService.getCos(req.queryParams("value"));
    ...
    private String getSinHandler(Request req, Response res) {
        String result = null;
        try {
            result = persistenceService.getSin(req.queryParams("value"));
        } catch (NanoSparkException e) {
            res.setError("There is no value to hello on the url, (i.e. /hello?value=example)");
        }
        return result;
    ...
    private String getTanHandler(Request req, Response res) {
        String result = null;
        try {
            result = persistenceService.getTan(req.queryParams("value"));
        } catch (NanoSparkException e) {
            res.setError("There is no value to hello on the url, (i.e. /hello?value=example)");
        }
        return result;
